...
def train(data, file_name, params, num_epochs=50, batch_size=128, train_temp=1, init=None):
    ...
    def fn(correct, predicted):
        return tensorflow.nn.softmax_cross_entropy_with_logits(labels=correct,
...
def train_distillation(data, file_name, params, num_epochs=50, batch_size=128, train_temp=1):
    ...
    with tensorflow.Session() as sess:
        y = sess.run(tensorflow.nn.softmax(predicted/train_temp))
        print(y)
        data.train_labels = y
...
train(CIFAR(), "models/cifar", [64, 64, 128, 128, 256, 256], num_epochs=10)
